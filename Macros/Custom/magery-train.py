# Name: Magery training to cap (fixed)
...
while not Dead('self') and Skill('Magery') < magery_cap:
    ...
    if Mana('self') > current_spell.mana_cost:
        ...
        timeout = 0
        while Mana('self') < MaxMana('self') and timeout < 120:
            # Her 2 saniyede bir dene — düşük skill'de çoğu başarısız olur
            if timeout % 2 == 0:
                UseSkill('Meditation')
            Pause(1000)
            timeout += 1
            # Yeterli mana varsa erken çık
            if Mana('self') >= current_spell.mana_cost:
                break
        if Mana('self') >= current_spell.mana_cost:
            continue
